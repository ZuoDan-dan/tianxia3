...
function EmailAutoComplete(options) {
    this.config = {
        ...
        mailArr        : ["@qq.com","@126.com","@163.com","@yeah.net"], //邮箱数组
    ...
    init: function(options){
        ...
        $(_config.targetCls).each(function(index,item){
            $(item).keyup(function(e){
                ...
                $(".pclear").eq(index).css("display","block");
                var target = e.target,
    ...
    _renderHTML: function(cfg) {
        ...
        if(curIndex > -1){
            ...
            if(_cache.onlyFlag) {
                ...
                $('.auto-tip',cfg.parentNode).css({'top':cfg.height});
            }
...
$(".pclear").eq(0).on({
    "click":function () {
        $("#email").val("");
        $(".pclear").eq(0).css("display","none");
    }});
$(".pclear").eq(1).on({
    "click": function () {
        $("#pwd").val("");
        $(".pclear").eq(1).css("display", "none");
    }
});
